...
- **Node1 (your-node1-hostname)**: 10.87.2.22 - Primary bootstrap node
- **Node2 (your-node2-hostname)**: 10.87.2.23 - Secondary node
...
**Usage**:
```bash
# On node2 (your-node2-hostname) - ALWAYS STOP NODE2 FIRST
./pre-patch-shutdown.sh

# Then on node1 (your-node1-hostname)
./pre-patch-shutdown.sh
```
...
**Usage**:
```bash
# On node1 (your-node1-hostname) - ALWAYS START NODE1 FIRST
./post-patch-startup.sh

# Wait for node1 to be PRIMARY, then on node2 (your-node2-hostname)
./post-patch-startup.sh
```
...
**Usage**:
```bash
# ONLY run on node1 (your-node1-hostname)
...
1. Stop both nodes:
   ```bash
   docker stop mariadb-galera-node2  # On your-node2-hostname
   docker stop mariadb-galera-node1  # On your-node1-hostname
   ```

2. Clean stale SST files on both nodes:
   ```bash
   # On your-node1-hostname (node1)
