...
```bash
# On node2 (your-node2-hostname) - FIRST
./pre-patch-shutdown.sh

# On node1 (your-node1-hostname) - SECOND
./pre-patch-shutdown.sh
```

### After Patching (Planned Maintenance)
```bash
# On node1 (your-node1-hostname) - FIRST
./post-patch-startup.sh

# On node2 (your-node2-hostname) - SECOND
./post-patch-startup.sh
```

### Emergency Recovery (After Crash)
```bash
# ONLY on node1 (your-node1-hostname)
...
### Shutdown Order
1. **Node2 FIRST** (your-node2-hostname)
2. **Node1 SECOND** (your-node1-hostname)

### Startup Order
1. **Node1 FIRST** (your-node1-hostname) - Bootstrap node
2. **Node2 SECOND** (your-node2-hostname) - Joining node
...
|------|----------|------------|----------------|------|
| Node1 | your-node1-hostname | 10.87.2.22 | mariadb-galera-node1 | Bootstrap/Primary |
| Node2 | your-node2-hostname | 10.87.2.23 | mariadb-galera-node2 | Secondary |
